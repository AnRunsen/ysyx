...
extern "C" void sim_exit(int code)
{
    ...
    printf("  BRANCH: \t%lu\t%f%%\n", itype_cnt.branch, (double)itype_cnt.branch / (double)instr_cnt * 100);
    printf("  JAL: \t\t%lu\t%f%%\n", itype_cnt.jal, (double)itype_cnt.jal / (double)instr_cnt * 100);
    printf("  JALR: \t%lu\t%f%%\n", itype_cnt.jalr, (double)itype_cnt.jalr / (double)instr_cnt * 100);
    printf("  LUI: \t\t%lu\t%f%%\n", itype_cnt.lui, (double)itype_cnt.lui / (double)instr_cnt * 100);
    printf("  AUIPC: \t%lu\t%f%%\n", itype_cnt.auipc, (double)itype_cnt.auipc / (double)instr_cnt * 100);
    printf("  OP: \t\t%lu\t%f%%\n", itype_cnt.op, (double)itype_cnt.op / (double)instr_cnt * 100);
    printf("  OP-IMM: \t%lu\t%f%%\n", itype_cnt.op_imm, (double)itype_cnt.op_imm / (double)instr_cnt * 100);
    printf("  LOAD: \t%lu\t%f%%\n", itype_cnt.load, (double)itype_cnt.load / (double)instr_cnt * 100);
    printf("  STORE: \t%lu\t%f%%\n", itype_cnt.store, (double)itype_cnt.store / (double)instr_cnt * 100);
    printf("  SYSTEM: \t%lu\t%f%%\n", itype_cnt.system, (double)itype_cnt.system / (double)instr_cnt * 100);
    printf("============================\n");
