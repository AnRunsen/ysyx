...
extern "C" void sim_exit(int code)
{
    ...
    printf("  BRANCH: \t%lu\t%f%%\t%lu\n", itype_cnt.branch, (double)itype_cnt.branch / (double)instr_cnt * 100, itype_clk_cnt.clk_branch);
    printf("  JAL: \t\t%lu\t%f%%\t%lu\n", itype_cnt.jal, (double)itype_cnt.jal / (double)instr_cnt * 100, itype_clk_cnt.clk_jal);
    printf("  JALR: \t%lu\t%f%%\t%lu\n", itype_cnt.jalr, (double)itype_cnt.jalr / (double)instr_cnt * 100, itype_clk_cnt.clk_jalr);
    printf("  LUI: \t\t%lu\t%f%%\t%lu\n", itype_cnt.lui, (double)itype_cnt.lui / (double)instr_cnt * 100, itype_clk_cnt.clk_lui);
    printf("  AUIPC: \t%lu\t%f%%\t%lu\n", itype_cnt.auipc, (double)itype_cnt.auipc / (double)instr_cnt * 100, itype_clk_cnt.clk_auipc);
    printf("  OP: \t\t%lu\t%f%%\t%lu\n", itype_cnt.op, (double)itype_cnt.op / (double)instr_cnt * 100, itype_clk_cnt.clk_op);
    printf("  OP-IMM: \t%lu\t%f%%\t%lu\n", itype_cnt.op_imm, (double)itype_cnt.op_imm / (double)instr_cnt * 100, itype_clk_cnt.clk_op_imm);
    printf("  LOAD: \t%lu\t%f%%\t%lu\n", itype_cnt.load, (double)itype_cnt.load / (double)instr_cnt * 100, itype_clk_cnt.clk_load);
    printf("  STORE: \t%lu\t%f%%\t%lu\n", itype_cnt.store, (double)itype_cnt.store / (double)instr_cnt * 100, itype_clk_cnt.clk_store);
    printf("  SYSTEM: \t%lu\t%f%%\t%lu\n", itype_cnt.system, (double)itype_cnt.system / (double)instr_cnt * 100, itype_clk_cnt.clk_system);
    printf("============================\n");
